...
def convert_bert_example(ex_idx, example: InputExample, tokenizer: BertTokenizer, max_seq_len, label2id):
    ...
    if ex_idx < 3:
        decode_text = tokenizer.decode(
            np.array(token_ids)[np.where(np.array(attention_masks) == 1)[0]].tolist())
...
def get_out(processor, json_path, args, label2id, mode):
    ...
    for i, example in enumerate(examples):
        ...
        if i == 5:
            break
    out = convert_examples_to_features(
        examples, args.max_seq_len, args.bert_dir, label2id)
    return out
...
if __name__ == '__main__':
    ...
    dev_out = get_out(processor, './data/raw_data/dev.json',
                      args, label2id, 'dev')
