        ...
        for raw_example in raw_examples:
            ...
            for event in event_list:
                ...
                for argument in arguments:
                    labels.append(
                        (event_type+'-'+argument['role'], argument['argument'], argument['argument_start_index']))
...
def convert_mrc_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    tmp_callback = (text_b, len(tokens_a) + 2, event_type)
    tmp_callback_labels = []
...
def get_data(processor, raw_data_path, json_file, mode, rolelabel2id, labels2query, args):
    ...
    data = convert_examples_to_features(
        examples, args.max_seq_len, args.bert_dir, rolelabel2id, labels2query)
...
if __name__ == '__main__':
    ...
    train_data = get_data(processor, raw_data_path, "train.json",
                          "train", rolelabel2id, labels2query, args)
    dev_data = get_data(processor, raw_data_path, "dev.json",
                        "dev", rolelabel2id, labels2query, args)
    test_data = get_data(processor, raw_data_path, "dev.json",
                         "test", rolelabel2id, labels2query, args)
